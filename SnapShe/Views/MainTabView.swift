...
struct DiscoverView: View {
    ...
    private let defaultInterval: TimeInterval = 6.0
    @State private var currentItemDuration: TimeInterval? = nil
    ...
    func makeUIViewController(context: Context) -> AVPlayerViewController {
        ...
        context.coordinator.durationObserver = player.currentItem?.observe(
            ...
            if duration.isFinite && duration > 0 {
                DispatchQueue.main.async {
                    context.coordinator.onDurationKnown?(duration)
                }
            }
        }
...
class DiscoverViewModel: ObservableObject {
    ...
    func load(token: String) async {
        ...
        if let response = try? await APIService.shared.fetchFeed(token: token),
           response.ok {
    ...
    func loadMore(token: String) async {
        // Backend aynı feed'i döndürdüğü için şimdilik no-op.
    }
}
